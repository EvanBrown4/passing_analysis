...
# Return the correct area of the pitch given a location (meters x meters).
# NOTE: I chose not to divide the pitch into equal boxes, but instead take into account the different areas of the pitch.
# The biggest reason for this was the penalty boxes. I wanted to make those their own boxes, as the mentality signifcantly
# changes when a player crosses into the penalty area.
def square(location):
    x = location[0]
    y = location[1]
    if x <= 18:
...
# Build the id for the event type.
def build(event):
    a = str(event['square'])
...
# Find the position group of a position.
def position(pos):
    if type(pos) == dict:
        id = pos['id']
        if id == 1: # Goalkeeper
            return 1
        elif id == 2: # Outside Back
            return 3
        elif id <= 5: # Center Back
            return 2
        elif id == 6: # Outside Back
            return 3
        elif id <= 8: # Winger
            return 6
        elif id <= 11: # Defensive Midfielder
            return 4
        elif id == 12: # Winger
            return 6
        elif id <= 15: # Center Midfielder
            return 5
        elif id <= 17: # Winger
            return 6
        elif id <= 20: # Attacking Midfielder
            return 7
        elif id == 21: # Winger
            return 6
        else: # Forward
...
# Helps enable me to remove the passing data to save memory after I have stored all the information I need separately
def body(action):
    if type(action) == dict:
...
# Remove files from previously run iterations

if os.path.exists(r'passing_data.json'):
    os.remove(r'passing_data.json')
for file in os.listdir(r'clean_files'):
    file_path = os.path.join(r'clean_files', file)
    if os.path.isfile(file_path):
        os.remove(r'clean_files\\' + file)

# Clean each file before merging them all
for file in os.listdir(r'statsbomb\data\events'):
    ...
    df = df.drop(['square', 'direction', 'alt', 'related_pressure'], axis = 1)

    # Save the cleaned data
    # Although these files aren't used elsewhere, I decided as a design choice to save them to storage instead of memory to help
    # the performance of the program.
    df.to_json(r'clean_files\\' + file[:-5] + r"_clean.json")


no_dup_files = set([])
for file_path in os.listdir(r'clean_files'):
    no_dup_files.add(r'clean_files\\' + file_path)
